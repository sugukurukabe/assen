...
// M2 Phase 1のスコープ（dispatch_assignmentをsubjectとする4docType）。M2 Phase 2で追加されたT2P docType（job_order_referral/一部dispatch_assignmentのt2p_individual_contract）はこのit.eachの対象外（別テストで検証）
// M2 Phase 1 scope (the 4 docTypes whose subject is dispatch_assignment). The T2P docTypes added in M2 Phase 2 (job_order_referral, plus t2p_individual_contract on dispatch_assignment) are out of scope for this it.each (covered by other tests)
// Lingkup M2 Phase 1 (4 docType dengan subjek dispatch_assignment). docType T2P yang ditambahkan di M2 Phase 2 (job_order_referral, plus t2p_individual_contract pada dispatch_assignment) di luar lingkup it.each ini (dicakup oleh test lain)
const PHASE1_DISPATCH_ASSIGNMENT_DOC_TYPES = [
...
describe("M2 Phase 1: 派遣3点書類（A2/A3/A10）＋labor_conditions_noticeのdocType別生成 / dispatch 3-document set (A2/A3/A10) plus labor_conditions_notice, generated per docType", () => {
  it("SUPPORTED_DOC_TYPESにM2 Phase 1・Phase 2の全docTypeが登録されている / SUPPORTED_DOC_TYPES registers every M2 Phase 1 and Phase 2 docType", () => {
    expect([...SUPPORTED_DOC_TYPES].sort()).toEqual(
      [
        ...PHASE1_DISPATCH_ASSIGNMENT_DOC_TYPES,
  ...
  it.each(PHASE1_DISPATCH_ASSIGNMENT_DOC_TYPES)(
    ...
    async (docType) => {
      ...
      const preview = await previewDocument(db, { tenantId, docType, subjectId: dispatchAssignmentId });

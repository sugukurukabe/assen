/**
 * inquiry.update：申込セット送付・書類受領・3日自動クローズ
 * inquiry.update: application-set send, document receipt, 3-day auto-close
 * inquiry.update: kirim paket, penerimaan dokumen, tutup otomatis 3 hari
 */
...
const inputSchema = {
  ...
  autoCloseStale: z.boolean().optional().describe("3日無応答なら自動クローズ（既定true） / Auto-close after 3 days (default true) / Tutup otomatis setelah 3 hari (default true)"),
};
...
export function registerInquiryUpdate(server: McpServer, context: ServiceContext): void {
  server.registerTool(
    "inquiry_update",
    {
      title: "問い合わせを更新する",
      description:
        "DM回答・正式申込セット送付・書類受領フラグを更新する。set_sentから3日たってもセット未完備なら自動クローズ（追いかけない）。 / Updates DM answers, application-set send, and receipt flags. Auto-closes if the set is still incomplete 3 days after set_sent (do not chase). / Memperbarui jawaban DM, pengiriman paket, dan flag penerimaan. Tutup otomatis jika paket belum lengkap 3 hari setelah set_sent (jangan dikejar).",
      inputSchema,
      annotations: {
        readOnlyHint: false,

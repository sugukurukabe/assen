...
const inputSchema = {
  ...
  businessFlag: z
    ...
    .optional()
    .describe("事業区分（スグクル／WIN／共通） / Business flag (Sugukuru/WIN/shared) / Klasifikasi bisnis (Sugukuru/WIN/bersama)"),
  placementPattern: z
    .enum(["P1", "P2", "P3", "P4", "P5"])
    .optional()
    .describe("成約パターンP1〜P5（省略時はtype/sourceから推定） / Placement pattern P1–P5 (inferred when omitted) / Pola penempatan P1–P5"),
    ...
    async (args) => {
      try {
        ...
        const result = await confirmJobOrderReferral(context.db, {
          ...
          type: args.type,
          businessFlag: args.businessFlag,
